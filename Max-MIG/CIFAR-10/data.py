...
class Im_EP(torch.utils.data.Dataset):
        ...
        for i, data in enumerate(data_loader):
            ...
            if Config.experiment_case == 1:
                for expert in range(Config.expert_num):
                    ep[i][expert] = int(np.random.choice(Config.num_classes, 1, p=self.as_expertise[expert][labels[i]])) # 第expert个人对第i个sample的预测类别
            ...
            if Config.experiment_case == 2:
                for expert in range(Config.senior):
                    ep[i][expert] = int(np.random.choice(Config.num_classes, 1, p=self.as_expertise[expert][labels[i]]))   # 选择预测的类别
                    right_data[i][expert][ep[i][expert]] = 1
                for expert in range(Config.senior, Config.expert_num):
                    right_data[i][expert][0] = 1         # 所有senior experts对sample的预测全是第一个类别，即为0
            ...
            if Config.experiment_case == 3:
                ...
                # 有junior_1个junior experts的预测和第0个(senior) expert保持一致
                for expert in range(Config.senior, Config.senior + Config.junior_1):
                    ep[i][expert] = ep[i][0]
                    right_data[i][expert][ep[i][expert]] = 1

                # 剩下的junior experts的预测和第2个(senior) expert保持一致
    ...
    def label_initial(self):
        """
        aggregate all experts' prediction
        (batch_size, M, C) --> (batch_size, C) --> (batch_size)
        """
